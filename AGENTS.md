...
- **Ship no third-party brand assets.** The icon under
  `custom_components/tripshot_tracker/brand/` is original work, generated by
  `tools/make_brand.py`, so the MIT grant covering this repository is honest.
  Vendoring a vendor logo would purport to sublicense something this project
  does not own. See `NOTICE`.
